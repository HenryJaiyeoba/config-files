...
const float BAR_WIDTH = 2.0; // width in pixels when Ghostty's cursor style is bar
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    // Ghostty reports the full cell bounds separately from the cursor style.
    float currentIsBar = 1.0 - step(0.5, abs(iCurrentCursorStyle.x - float(CURSORSTYLE_BAR)));
    float previousIsBar = 1.0 - step(0.5, abs(iPreviousCursorStyle.x - float(CURSORSTYLE_BAR)));
    currentCursor.z = mix(currentCursor.z, normalize(vec2(BAR_WIDTH), 0.).x, currentIsBar);
    previousCursor.z = mix(previousCursor.z, normalize(vec2(BAR_WIDTH), 0.).x, previousIsBar);
